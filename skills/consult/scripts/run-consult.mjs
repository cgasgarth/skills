...
async function actualProjectLabel(tab, requested) {
  ...
  const labels = await tab.playwright.evaluate((target) => {
    const values = [];
    for (const element of Array.from(document.querySelectorAll("button,[role='button']")).slice(0, 120)) {
      const text = (element.textContent || "").trim().replace(/\s+/g, " ");
      if (text && text.normalize("NFKC").toLocaleLowerCase() === target) values.push(text);
    }
...
async function openProject(tab, project) {
  ...
  const snapshot = await tab.playwright.domSnapshot();
  const composerName = `New chat in ${project}`;
...
async function ensurePro(tab) {
  const pro = tab.playwright.getByRole("button", { name: "Pro", exact: true });
  if (await visible(pro)) return;

  const selector = tab.playwright.getByRole("button", { name: "Model selector", exact: true });
  if (!await visible(selector)) throw new Error("Pro is absent and the Model selector is unavailable.");
  await selector.click();
  await tab.playwright.domSnapshot();
  const proOption = tab.playwright.getByText("Pro", { exact: true });
  await (await requireOne(proOption, "Pro option")).click();
  await tab.playwright.domSnapshot();
  const model = tab.playwright.getByText("GPT-5.6 Sol", { exact: true });
  await (await requireOne(model, "GPT-5.6 Sol option")).click();
  await tab.playwright.domSnapshot();
...
export async function startConsult({ iab, project, prompt, send = true, createImage = false, aspectRatio = null }) {
  ...
  if (!send) return { status: "setup_verified_not_sent", project, tab };
